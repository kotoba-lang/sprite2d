(ns kotoba.sprite2d
  "2D characters as EDN — 'hiccup for sprites'. A sprite is a vector of primitive shapes in
   ...
   kotoba.sprite2d.layout — CLJ-tested on the JVM (camera, W/S orientation, variant swap,
   depth). This painter only turns that list + the trees into canvas ops."
  (:require [kotoba.sprite2d.layout :as layout]))
  ...
  (let [cv (.-canvas ctx)]
   ...
   (do
     (defn- browser-only [f data]
       (throw (ex-info (str "kotoba.sprite2d/" f " is a browser ClojureScript Canvas2D executor")
                       (merge {:namespace 'kotoba.sprite2d :platform :clj} data))))

     (defn draw-sprite! [& args] (browser-only "draw-sprite!" {:args args}))
     (defn prim! [& args] (browser-only "prim!" {:args args}))
     (defn draw-2d! [& args] (browser-only "draw-2d!" {:args args}))
     (defn draw-portrait! [& args] (browser-only "draw-portrait!" {:args args}))))
